...
将 `DyDanmaku-neoforge-1.21.1-0.1.7.2.jar` 放入客户端的 `mods` 目录。进入游戏后按 `F7` 打开控制界面。Mod 默认启用基于抖音官方 OpenAPI 的 bridge WSS 接入；原有的直播间 URL/房间号直连能力仍然保留，可在“更多设置 → 接入设置”中关闭官方接入后使用。
...
```text
build/libs/DyDanmaku-neoforge-1.21.1-0.1.7.2.jar
```
