...
try {
    ...
    Write-Host "[DyDanmaku] Forge 1.21.1 使用 JAVA_HOME=$env:JAVA_HOME"
    if ($Clean) {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Forge 1.21.1 构建失败，退出码：$LASTEXITCODE"
    }
} finally {
