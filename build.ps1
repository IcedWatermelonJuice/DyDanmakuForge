...
if ([string]::IsNullOrWhiteSpace($jdkHome)) {
    throw 'JAVA_17_HOME is not set. Point it to the JDK 17 directory.'
}
$jdkHome = [Environment]::ExpandEnvironmentVariables($jdkHome).TrimEnd([char[]]@('\', '/'))
$javaExe = Join-Path $jdkHome 'bin\java.exe'
if (-not (Test-Path -LiteralPath $javaExe -PathType Leaf)) {
    throw "Invalid JAVA_17_HOME; java.exe was not found: $javaExe"
}
...
try {
    ...
    Write-Host "[DyDanmaku] Forge 1.20.1 JAVA_HOME=$env:JAVA_HOME"
    if ($Clean) {
